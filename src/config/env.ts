import dotenv from 'dotenv';
dotenv.config();


export const DATABASE_URL = process.env.DATABASE_URL;
export const CASSANDRA_HOST = process.env.CASSANDRA_HOST;
export const CASSANDRA_DATACENTER = process.env.CASSANDRA_DATACENTER;
export const CASSANDRA_KEYSPACE = process.env.CASSANDRA_KEYSPACE;
export const CASSANDRA_PORT = Number(process.env.CASSANDRA_PORT);
export const CASSANDRA_USERNAME = process.env.CASSANDRA_USERNAME;
export const CASSANDRA_PASSWORD = process.env.CASSANDRA_PASSWORD;
export const REDIS_HOST = process.env.REDIS_HOST;
export const REDIS_PORT = Number(process.env.REDIS_PORT);
export const PORT = Number(process.env.PORT);
export const NODE_ENV = process.env.NODE_ENV;
export const JWT_ACCESS_SECRET = process.env.JWT_ACCESS_SECRET;
export const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET;
export const ACCESS_TOKEN_EXPIRES_IN = process.env.ACCESS_TOKEN_EXPIRES_IN;
export const REFRESH_TOKEN_EXPIRES_IN = process.env.REFRESH_TOKEN_EXPIRES_IN;
export const COOKIE_DOMAIN = process.env.COOKIE_DOMAIN;
export const FRONTEND_ORIGIN = process.env.FRONTEND_ORIGIN;
export const COOKIE_SECURE = NODE_ENV === 'production';
export const FROM_EMAIL = process.env.FROM_EMAIL;
export const RESEND_API_KEY = process.env.RESEND_API_KEY;
export const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID;
export const MINIO_ENDPOINT = process.env.MINIO_ENDPOINT;
export const MINIO_PORT = Number(process.env.MINIO_PORT);
export const MINIO_USE_SSL = process.env.MINIO_USE_SSL === 'true';
export const MINIO_ACCESS_KEY = process.env.MINIO_ACCESS_KEY;
export const MINIO_SECRET_KEY = process.env.MINIO_SECRET_KEY;
export const MINIO_BUCKET_NAME = process.env.MINIO_BUCKET_NAME;
export const MINIO_PRESIGNED_EXPIRY_SECONDS = Number(process.env.MINIO_PRESIGNED_EXPIRY_SECONDS || 60);
export const MEDIA_MAX_UPLOAD_BYTES = Number(process.env.MEDIA_MAX_UPLOAD_BYTES || 25 * 1024 * 1024);
export const UPLOAD_RATE_LIMIT_PER_MINUTE = Number(process.env.UPLOAD_RATE_LIMIT_PER_MINUTE || 20);
export const VAPID_PUBLIC_KEY = process.env.VAPID_PUBLIC_KEY;
export const VAPID_PRIVATE_KEY = process.env.VAPID_PRIVATE_KEY;
export const VAPID_SUBJECT = process.env.VAPID_SUBJECT;
export const PUSH_NOTIFICATION_ASSETS_BASE_URL = process.env.PUSH_NOTIFICATION_ASSETS_BASE_URL || '/assets';
export const PUSH_NOTIFICATION_ICON_PATH = process.env.PUSH_NOTIFICATION_ICON_PATH || `${PUSH_NOTIFICATION_ASSETS_BASE_URL}/notification-icon.svg`;
export const PUSH_NOTIFICATION_BADGE_PATH = process.env.PUSH_NOTIFICATION_BADGE_PATH || `${PUSH_NOTIFICATION_ASSETS_BASE_URL}/notification-badge.svg`;
